...
namespace gazebo
{
	...
	std::string ending_tag = "<plugin name='lmaze_controller_plugin' filename='/homes/gkumar/rl/environments/PrivateModelDevelopment5/build/liblmaze_controller_plugin.so'> \
...
//	        for (int i=MAZE_SIZE-1; i >= 0; i--){ // Outer-Most Loop
	        for (int i=0; i < MAZE_SIZE; i++){ // Outer-Most Loop
	            ...
	            for (int j=0; j < MAZE_SIZE; j++){
	                if (line.at(j) == '|' || line.at(j) == '_'){
	                    resource_uri = "/homes/gkumar/rl/environments/PrivateModelDevelopment5/models/cube.dae";
	                ...
	                tempSDF = tempSDF
	                    ...
	                    resource_uri = "/homes/gkumar/rl/environments/PrivateModelDevelopment5/models/cubePit.dae";
	...
	void LmazePlugin::OnReset(const std_msgs::Bool::ConstPtr& msg){
	    ...
	    //if(resetMsg) {
			...
			World->SetPaused(1);	// Pause
			World->Reset();
			World->SetPaused(0);		// Play
		//}
	}
